	...
	public static void performSearch(Stack<SearchNode> s, boolean d)
	{
		...
		while (!s.isEmpty()) // while the queue is not empty
		{
			...
			// it.
			{
				...
				while (tempNode.getParent() != null)//backtracking using parent 
				{
					solutionPath.push(tempNode);
					tempNode = tempNode.getParent();
				}
				solutionPath.push(tempNode);//root node....
				...
				System.out.println( tempNode.getCost());
				//if (d)
				//{
		...
		System.out.println("Error! No solution found!..please check your input...");
	}
}
