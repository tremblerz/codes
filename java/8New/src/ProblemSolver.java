...
public class ProblemSolver
{
	...
	public static void main(String[] args)
	{
		...
		if (args.length > 1) // We will run with 8puzzle
		{
			int[] startingStateBoard;
			if (searchType.equals("dfs")) // Use DFSearch.java
			{
				int i=0;
				for(i=0;i<1000;i++)
				{
					startingStateBoard = new int[]{-1,-1,-1,-1,-1,-1,-1,-1,-1};
					int j=0;
					for(j=0;j<=9;j++)
					{
						int k = rand.nextInt(9);
						if(startingStateBoard[k]==-1)
							startingStateBoard[k]=j;
						else
							j--;
					}
